...
CREATE POLICY "Trainers can view client habits"
    ...
    USING (
        EXISTS (
            ...
            WHERE trainer_clients.client_id = habits.user_id
...
CREATE POLICY "Trainers can view client habit completions"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM trainer_clients
            WHERE trainer_clients.client_id = habit_completions.user_id
            AND trainer_clients.trainer_id = auth.uid()
            AND trainer_clients.status = 'active'
        )
    );
...
CREATE POLICY "Trainers can view client habit progress"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM trainer_clients
            WHERE trainer_clients.client_id = habit_progress.user_id
            AND trainer_clients.trainer_id = auth.uid()
            AND trainer_clients.status = 'active'
        )
    );
