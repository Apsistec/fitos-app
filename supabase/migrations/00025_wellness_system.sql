...
CREATE POLICY "Trainers can view client wellness screenings"
    ...
    USING (
        EXISTS (
            ...
            WHERE trainer_clients.client_id = wellness_screenings.user_id
...
CREATE POLICY "Trainers can view client wellness workout plans"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM trainer_clients
            WHERE trainer_clients.client_id = wellness_workout_plans.user_id
            AND trainer_clients.trainer_id = auth.uid()
            AND trainer_clients.status = 'active'
        )
    );
...
CREATE POLICY "Trainers can view client wellness workout sessions"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM trainer_clients
            WHERE trainer_clients.client_id = wellness_workout_sessions.user_id
            AND trainer_clients.trainer_id = auth.uid()
            AND trainer_clients.status = 'active'
        )
    );
...
CREATE POLICY "Trainers can view client crisis resource access"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM trainer_clients
            WHERE trainer_clients.client_id = wellness_crisis_resource_access.user_id
            AND trainer_clients.trainer_id = auth.uid()
            AND trainer_clients.status = 'active'
        )
    );
...
CREATE POLICY "Trainers can view client wellness progress"
    ...
    USING (
        EXISTS (
            SELECT 1 FROM trainer_clients
            WHERE trainer_clients.client_id = wellness_progress.user_id
            AND trainer_clients.trainer_id = auth.uid()
            AND trainer_clients.status = 'active'
        )
    );
